...
const MenuItem: React.FC<{ name: string; path: string }> = ({ name, path }) => {
  ...
  return (
    <Link
      ...
      className="rounded-lg p-2 leading-none text-neutral-500 data-[active='true']:text-neutral-900  hover:text-neutral-900 dark:text-neutral-400 data-[active='true']:dark:text-teal-400 dark:hover:text-neutral-200"
    >
      {name}
    </Link>
  )
