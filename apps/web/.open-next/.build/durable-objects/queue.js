...
const IgnorableError = class extends Error {
...
const RecoverableError = class extends Error {
  __openNextInternal = true;
  canIgnore = true;
  logLevel = 1;
  constructor(message) {
...
const isDownplayedErrorLog = (errorLog) => DOWNPLAYED_ERROR_LOGS.some((downplayedInput) => downplayedInput.clientName === errorLog?.clientName && downplayedInput.commandName === errorLog?.commandName && (downplayedInput.errorName === errorLog?.error?.name || downplayedInput.errorName === errorLog?.error?.Code));
function error(...args) {
...
// ../../node_modules/.bun/@opennextjs+cloudflare@1.16.2+73e503e3033d6e27/node_modules/@opennextjs/cloudflare/dist/api/durable-objects/queue.js
import { DurableObject } from "cloudflare:workers";
const DEFAULT_MAX_REVALIDATION = 5;
const DEFAULT_REVALIDATION_TIMEOUT_MS = 1e4;
const DEFAULT_RETRY_INTERVAL_MS = 2e3;
const DEFAULT_MAX_RETRIES = 6;
const DOQueueHandler = class extends DurableObject {
  ...
  constructor(ctx, env) {
    ...
    this.maxRevalidations = env.NEXT_CACHE_DO_QUEUE_MAX_REVALIDATION ? Number.parseInt(env.NEXT_CACHE_DO_QUEUE_MAX_REVALIDATION) : DEFAULT_MAX_REVALIDATION;
    this.revalidationTimeout = env.NEXT_CACHE_DO_QUEUE_REVALIDATION_TIMEOUT_MS ? Number.parseInt(env.NEXT_CACHE_DO_QUEUE_REVALIDATION_TIMEOUT_MS) : DEFAULT_REVALIDATION_TIMEOUT_MS;
    this.revalidationRetryInterval = env.NEXT_CACHE_DO_QUEUE_RETRY_INTERVAL_MS ? Number.parseInt(env.NEXT_CACHE_DO_QUEUE_RETRY_INTERVAL_MS) : DEFAULT_RETRY_INTERVAL_MS;
    this.maxRetries = env.NEXT_CACHE_DO_QUEUE_MAX_RETRIES ? Number.parseInt(env.NEXT_CACHE_DO_QUEUE_MAX_RETRIES) : DEFAULT_MAX_RETRIES;
    ...
    try {
      ...
      response = await this.service.fetch(`${protocol}://${host}${url}`, {
        ...
        headers: {
          ...
          "x-prerender-revalidate": "fdc0f895862f325e0be8db8ad383cd23",
          "x-isr": "1"
        },
  ...
  async addToFailedState(msg) {
    ...
    if (existingFailedState) {
      ...
      const nextAlarmMs = Date.now() + 2 ** (existingFailedState.retryCount + 1) * this.revalidationRetryInterval;
      updatedFailedState = {
        ...existingFailedState,
    ...
    if (!this.disableSQLite) {
      this.sql.exec("INSERT OR REPLACE INTO failed_state (id, data, buildId) VALUES (?, ?, ?)", msg.MessageDeduplicationId, JSON.stringify(updatedFailedState), "A5t6Z7yKSP7YV5klSzW1h");
    }
    await this.addAlarm();
  }
  async addAlarm() {
  ...
  async initState() {
    ...
    this.sql.exec("DELETE FROM failed_state WHERE buildId != ?", "A5t6Z7yKSP7YV5klSzW1h");
    this.sql.exec("DELETE FROM sync WHERE buildId != ?", "A5t6Z7yKSP7YV5klSzW1h");
