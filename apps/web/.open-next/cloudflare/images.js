...
const define_IMAGES_DEVICE_SIZES_default = [640, 750, 828, 1080, 1200, 1920, 2048, 3840];

// <define:__IMAGES_FORMATS__>
const define_IMAGES_FORMATS_default = ["image/webp"];

// <define:__IMAGES_IMAGE_SIZES__>
const define_IMAGES_IMAGE_SIZES_default = [32, 48, 64, 96, 128, 256, 384];

// <define:__IMAGES_LOCAL_PATTERNS__>
const define_IMAGES_LOCAL_PATTERNS_default = [{ pathname: "^(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$))(?:(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$)).)*?)\\/?)$", search: "" }];

// <define:__IMAGES_QUALITIES__>
const define_IMAGES_QUALITIES_default = [75];

// <define:__IMAGES_REMOTE_PATTERNS__>
const define_IMAGES_REMOTE_PATTERNS_default = [{ hostname: "^(?:^(?:avatar\\.vercel\\.sh)$)$", pathname: "^(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$))(?:(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$)).)*?)\\/?)$" }, { protocol: "https", hostname: "^(?:^(?:cdn\\.simpleicons\\.org)$)$", pathname: "^(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$))(?:(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$)).)*?)\\/?)$" }, { protocol: "https", hostname: "^(?:^(?:ui\\.shadcn\\.com)$)$", pathname: "^(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$))(?:(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$)).)*?)\\/?)$" }, { protocol: "https", hostname: "^(?:^(?:cursor\\.com)$)$", pathname: "^(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$))(?:(?:(?!(?:^|\\/)\\.{1,2}(?:\\/|$)).)*?)\\/?)$" }];
...
// ../../node_modules/.bun/@opennextjs+cloudflare@1.16.2+73e503e3033d6e27/node_modules/@opennextjs/cloudflare/dist/cli/templates/images.js
...
function validateWidthQueryParameter(requestURL) {
  ...
  const width = Number.parseInt(widthQueryValue, 10);
...
function validateQualityQueryParameter(requestURL) {
  ...
  const quality = Number.parseInt(qualityQueryValue, 10);
