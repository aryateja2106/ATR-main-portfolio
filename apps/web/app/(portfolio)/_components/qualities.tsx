	...
	return (
		<section id="qualities" className="w-full mt-28">
			<h2 className="flex items-center text-3xl font-bold mb-8">
				<span className="mr-3 text-foreground">What I</span>
				<span className="text-teal-400 mr-4">Bring to the Table</span>
				<div className="h-px bg-gradient-to-r from-teal-500/50 to-transparent grow" />
			</h2>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				{qualities.map((quality, index) => (
					...
					>
						<motion.div
							className={
								"glass-panel h-full p-6 hover:border-teal-500/30 transition-all duration-300 flex flex-col overflow-hidden relative group rounded-xl"
							}
							whileHover={{
								y: -5,
						...
						>
							{/* Subtle ambient glow instead of hard gradient */}
							<div
								className={`absolute inset-0 bg-gradient-to-br ${quality.color} opacity-10 group-hover:opacity-20 transition-opacity duration-500`}
							/>
							...
							<div className="relative z-10 flex flex-col h-full">
								...
								{/* Title */}
								<h3 className="text-xl font-semibold mb-3 text-teal-400 group-hover:text-teal-300 transition-colors">
									{quality.title}
								</h3>
								...
								<div className="absolute top-0 right-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
									<div className="w-2 h-2 rounded-full bg-teal-500/50 blur-[2px]" />
								</div>
							</div>
						</motion.div>
					</motion.div>
