...
const projectsData: Project[] = [
	{
		...
		description:
			"Agent credit economy marketplace. Built branch-and-leaf agent marketplace at Nevermined hackathon with 7 services, 21+ agent-to-agent purchases using x402 payment protocol.",
		techStack: ["TypeScript", "Bun", "x402", "USDC", "Base Sepolia", "MCP"],
		githubLink: "https://github.com/aryateja2106/cloudagi",
		metrics: "21+ agent-to-agent purchases from 5 teams",
	},
	{
		id: "lesearch-ai",
		title: "LeSearch AI",
		...
		statusVariant: "active",
		description:
			"AI research platform with multi-model orchestration. Docker MCP server exposing 77+ containerized tools, Agent Bridge MCP connecting 4 coding agents, and iOS companion app.",
		techStack: ["Next.js", "Docker", "MCP", "Claude", "Gemini", "Supabase"],
		githubLink: "https://github.com/aryateja2106/lesearch-app",
		metrics: "100+ researchers at 4 universities, 87% retention",
	},
	{
		id: "lecoder-mconnect",
		...
		statusVariant: "active",
		description:
			"iOS app + CLI giving mobile control over Claude Code, Gemini CLI, and local models. Shipped to the App Store and published on npm. Real-time WebSocket bridge.",
		techStack: ["Swift", "TypeScript", "WebSockets", "Node PTY", "Cloudflare"],
		githubLink: "https://github.com/aryateja2106/lecoder-mconnect",
		liveLink: "https://lecoder.lesearch.ai",
		metrics: "Shipped to App Store + npm",
	},
	{
		id: "karna",
		title: "Karna",
		...
		statusVariant: "active",
		description:
			"Local-first AI coding agent. 13-crate Rust workspace with 3-layer security pipeline, AES-256-GCM encrypted vault, zero unsafe Rust. Ratatui TUI, WebSocket gateway.",
		techStack: ["Rust", "Ratatui", "WebSockets", "AES-256-GCM", "Ollama"],
	...
	},
	{
		...
		description:
			"Complete PyTorch reproduction of 3 Google Research papers on neural memory (TITANS, MIRAS, Nested Learning). Published research-reproduction Claude Code skill.",
		techStack: ["Python", "PyTorch", "CUDA", "Research"],
		githubLink: "https://github.com/aryateja2106/neural-memory-reproduction",
		metrics: "3 papers reproduced, 52+ tests, 87% coverage",
	},
	{
		id: "lecoder-cgpu",
		...
		statusVariant: "active",
		description:
			"Production CLI giving programmatic control over Google Colab runtimes. Run notebooks, execute code on free cloud GPUs (T4/V100/A100) without leaving the terminal.",
		techStack: ["TypeScript", "Bun", "Google Colab", "CLI"],
		githubLink: "https://github.com/aryateja2106/lecoder-cgpu",
		liveLink: "https://www.npmjs.com/package/lecoder-cgpu",
		metrics: "Published on npm",
	},
	{
		id: "aryaos",
		title: "AryaOS",
		...
		statusVariant: "active",
		description:
			"macOS-style interactive portfolio at os.aryateja.com. Windowed desktop with draggable apps, Dock, and integrated terminal for live agent demos.",
		techStack: ["Next.js", "Tailwind CSS", "Framer Motion", "xterm.js"],
		liveLink: "https://os.aryateja.com",
	},
];
...
const Badge: FC<BadgeProps> = ({ variant, children, className = "" }) => {
	...
	const variantClasses = {
		...
		"neutral-outline":
			"bg-transparent text-neutral-400 border border-neutral-700 hover:bg-neutral-800/50", // Simple outline, subtle hover
	};
	const selectedVariant =
		variantClasses[variant] || variantClasses["neutral-outline"];
...
const Button: FC<ButtonProps> = ({
	...
	const baseClasses =
		"inline-flex items-center justify-center rounded-md font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-neutral-500/40 focus:ring-offset-2 focus:ring-offset-neutral-950";
	const variantClasses = {
		default: "bg-neutral-700/80 text-neutral-100 hover:bg-neutral-700",
		outline:
			"border border-neutral-700 bg-transparent hover:bg-neutral-800/60 text-neutral-300 hover:text-neutral-100",
		ghost:
...
const AccordionTrigger: FC<AccordionTriggerProps> = ({
	...
	<button
		type="button"
		className={`flex w-full items-center justify-between py-3 text-sm font-medium transition-all hover:text-neutral-100 text-neutral-400 ${className}`}
		onClick={onClick}
	>
		{children}
		<motion.svg
...
const githubBodyVariants: Variants = {
	normal: {
		opacity: 1,
		pathLength: 1,
		scale: 1,
...
const githubTailVariants: Variants = {
...
const GithubIcon = forwardRef<GithubIconHandle, GithubIconProps>(
	({ onMouseEnter, onMouseLeave, className, size = 16, ...props }, ref) => {
		...
		return (
			...
			// biome-ignore lint/a11y/noStaticElementInteractions: Icon animation only, parent is interactive
			<div
				className={cn(
			...
			>
				...
				>
					...
					<motion.path
						variants={githubBodyVariants}
						initial="normal"
						animate={bodyControls}
						d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"
					/>
					<motion.path
						variants={githubTailVariants}
						initial="normal"
...
const Card: FC<CardProps> = ({ children, className = "" }) => (
	<div
		className={`bg-neutral-900/60 backdrop-blur-sm border border-neutral-800 rounded-lg shadow-md overflow-hidden flex flex-col ${className}`}
	>
		{children}
	</div>
);
...
const CardTitle: FC<CardTitleProps> = ({ children, className = "" }) => (
	<h3
		className={`text-base md:text-lg font-semibold text-neutral-100 ${className}`}
	>
		{children}
	</h3>
);
...
const CardContent: FC<CardContentProps> = ({ children, className = "" }) => (
	<div className={`px-5 md:px-6 pb-5 md:pb-6 grow ${className}`}>
		{children}
	</div>
);
			...
			>
				<Card className="min-h-[300px] h-full hover:border-neutral-600/80 transition-all duration-300 ease-in-out hover:shadow-lg hover:shadow-teal-500/10">
					...
					<CardContent className="flex flex-col">
						{/* Metrics Section - Highlighted */}
						{project.metrics && (
							<div className="mb-3 px-3 py-2 bg-teal-500/10 border border-teal-500/20 rounded-md">
						...
						<div className="h-[72px] mb-4 overflow-hidden">
							{" "}
							{/* Approx 3 lines height - adjusted for metrics */}
							<p
								className={cn(
									"text-sm text-neutral-400 line-clamp-3",
									project.metrics ? "line-clamp-2" : "line-clamp-3",
								)}
							>
								{project.description}
							</p>
						...
						<div className="flex justify-start items-center gap-2 mb-4 pt-1 h-[32px]">
							{" "}
							{/* Fixed height for one row of sm buttons */}
							{project.githubLink && (
								<Button
									variant="outline"
									size="sm"
								...
								>
									<span className="flex items-center">
										...
										<GithubIcon size={14} className="mr-1.5 size-3.5" />{" "}
										{/* Adjusted size */}
										Code
									</span>
								</Button>
			...
			<div className="container mx-auto px-4 md:px-6">
				...
				>
					...
					<p className="text-lg md:text-xl text-neutral-400 max-w-3xl mx-auto">
						A selection of work I&apos;m involved in, from active development
						and experiments to contributions and forks.
					</p>
				</motion.div>
