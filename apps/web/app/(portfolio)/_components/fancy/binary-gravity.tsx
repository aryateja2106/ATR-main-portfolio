...
import { Gravity, MatterBody } from './physics/cursor-attractor-and-gravity';
...
export function BinaryGravity({ className = '' }: BinaryGravityProps) {
  ...
  useEffect(() => {
    const generateTrail = () => {
      ...
      if (now - lastTrailTime.current > 150) {
        // Limit rate of particle creation
        const newParticle = {
          id: now + Math.random(),
  ...
  return (
    <div
      ref={containerRef}
      className={`size-full absolute inset-0 overflow-hidden ${className}`}
    >
      <button
        className="size-full absolute inset-0 cursor-default bg-transparent"
